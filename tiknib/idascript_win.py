...
import sys
sys.path.append("..")
from tiknib.utils import system, get_file_type_win, do_multiprocess
...
class IDAScript:
    ...
    def run_helper(self, input_fname):
        ...
        arch = get_file_type_win(input_fname,use_str=True)
        if arch is None:
        ...
        if arch.find("_32") != -1:
            ida = self.idapath + "/ida.exe"
        else:
            ida = self.idapath + "/ida64.exe"

        # >= IDA Pro v7.4 use "idat" instead of "idal"
        if not os.path.exists(ida):
            ida = ida.replace('ida.exe', 'idat.exe')
    ...
    def run(self, input_path):
        ...
        res = do_multiprocess(self.run_helper, elfs, chunk_size=1, threshold=1) # linux下的多线程
        # executor = ThreadPoolExecutor(max_workers=4) # win 线程池
        # for res in tqdm(executor.map(self.run_helper, elfs)):
        #     print(res)
        # for i in tqdm(range(len(elfs))):
        #     input_fname, state = self.run_helper(elfs[i])
        #     print(input_fname, state)
