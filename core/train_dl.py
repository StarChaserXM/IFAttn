...
from module.CodeAttention.Optim import ScheduledOptim
...
def att_weight_analysis(att,features):
    ...
    fea_ids = heapq.nlargest(10, range(len(att)), att.take)
...
def do_train(opts):
    ...
    for config_fname in tqdm(config_fname_list):
        ...
        topk_list = calc_topK(src_all, dst_all, lable_all, k_list, len(test_funcs)+len(valid_funcs))
