...
def gettmpdir():
    tmpdir = os.path.join("/tmp", "module_tmp", randstr(10))
...
def get_cache_fname(fname="", cache_dir=".module_cache"):
    if not fname:
        fname = randstr(10)
    if not cache_dir:
        cache_dir = os.path.join("./", ".module_cache")
...
def load_cache(fname="", cache_dir=".module_cache"):
...
def store_cache(data, fname="", cache_dir=".module_cache"):
    cache_fname = get_cache_fname(fname=fname, cache_dir=cache_dir)
...
def get_file_type_win(fname, use_str=False):
    head, tail = ntpath.split(fname)
    return "_".join(tail.split('_')[-4:-2])
