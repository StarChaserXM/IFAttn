...
from utils import demangle, get_arch, init_idc, parse_fname, store_func_data
...
def get_type(addr):
    ...
    func_type = idaapi.print_tinfo("", 0, 0, idaapi.PRTYPE_1LINE, tif, "", "")
    ret_type = idaapi.print_tinfo("", 0, 0, idaapi.PRTYPE_1LINE, funcdata.rettype, "", "")
    args = []
    for i in range(funcdata.size()):
        arg_type = idaapi.print_tinfo("", 0, 0, idaapi.PRTYPE_1LINE, funcdata[i].type, "", "")
...
def main():
    ...
    for idx, addr in enumerate(list(idautils.Functions())):
        ...
        stack_size = idc.get_frame_size(addr)
        ...
        if func_name in callee_map:
            imported_callees = list(
                filter(lambda x: idc.get_segm_name(x[1]) != idc.get_segm_name(addr), callee_map[func_name])
            )
