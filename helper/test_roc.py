...
# 设置有多少个特征就循环多少轮，每一轮尝试向备选特征表中添加使得roc更大的特征
# 一旦发现再添加任何特征都不能提升roc时训练结束
# python test_roc.py --input_list /mnt/JiangS/CodeTransformer/BCSA/TikNib/helper/input/linux/done_list_a2ps_all_clang-4.0_clang-obfus_x86_64_all_a2ps.elf.txt --config /mnt/JiangS/CodeTransformer/BCSA/TikNib/config/gnu_type/config_gnu_normal_obfus_fla_type.yml --type
...
def do_test(opts):
    ...
    for config_fname in tqdm(config_fname_list):
        ...
        for trial_idx, (train_func_keys, test_func_keys) in enumerate(kf.split(func_keys)):
            ...
            test_topk_list = calc_topK(copy.deepcopy(test_funcs),selected_feature_indices,k_list) # 拷贝字典不能用copy(浅拷贝相当于引用),采样test_funcs个函数计算topk
            # test_topk_list = [0.0,0.0]
